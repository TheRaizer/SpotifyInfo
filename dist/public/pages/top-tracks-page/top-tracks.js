...
const trackActions = (function () {
    ...
    const cardActionsHandler = new card_actions_js_1.default(MAX_VIEWABLE_CARDS);
    const selections = {
        numViewableCards: DEFAULT_VIEWABLE_CARDS,
        term: "short_term",
    };
    function addTrackCardListeners(trackArr) {
        let trackCards = Array.from(document.getElementsByClassName(config_js_1.config.CSS.CLASSES.track));
    ...
    function loadFeatures(trackArr) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let res = yield axios
                .get(config_js_1.config.URLs.getTrackFeatures + ids)
                .catch((err) => {
                throw err;
            });
    ...
    function retrieveTracks(trackArr) {
        return __awaiter(this, void 0, void 0, function* () {
            let { res, err } = yield (0, config_js_1.promiseHandler)(axios.get(config_js_1.config.URLs.getTopTracks + selections.term));
            if (err) {
                throw new Error(err);
            }
            (0, track_js_1.generateTracksFromData)(res.data, trackArr);
            yield (0, config_js_1.promiseHandler)(loadFeatures(trackArr));
        });
    }
    return {
...
const displayCardInfo = (function () {
    const tracksContainer = document.getElementById(config_js_1.config.CSS.IDs.trackCardsContainer);
    ...
    function generateCards(trackArr) {
        (0, config_js_1.removeAllChildNodes)(tracksContainer);
        let cardHtmls = [];
        ...
        config_js_1.animationControl.animateAttributes("." + config_js_1.config.CSS.CLASSES.rankCard, config_js_1.config.CSS.CLASSES.appear, 25);
        return cardHtmls;
    }
...
class Feature {
    ...
    calculateStd() {
        let sum = 0;
        this.data.forEach((val) => {
            let distance = Math.pow(Math.abs(val - this.mean), 2);
            sum += distance;
        });
...
const featureManager = (function () {
    const tracksChartEl = document.getElementById(config_js_1.config.CSS.IDs.tracksChart);
    const FEAT_HIGH = 60;
    const FEAT_LOW = 40;
    const charts = {
        tracksChart: null,
    ...
    function updateFeaturesAttrs(featArr) {
        ...
        keys.forEach((key) => {
            ...
            if (key != FEATURE_KEYS.POPULARITY) {
                ...
                feat.setData(featArr.map((features) => Math.round(features[feat.featKey] * 100)));
            }
        });
    }
    ...
    function generateTracksChart(trackObjs) {
        ...
        charts.tracksChart = new Chart(tracksChartEl, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: (0, config_js_1.capitalizeFirstLetter)(selections.feature.featKey),
    ...
    function updateTracksChart(trackObjs) {
        ...
        chart.data.datasets[0].label = (0, config_js_1.capitalizeFirstLetter)(selections.feature.featKey);
        chart.update();
    }
    ...
    function updateTracksChartInfo() {
        ...
        const featDef = document.getElementById(config_js_1.config.CSS.IDs.featDef);
        const featAverage = document.getElementById(config_js_1.config.CSS.IDs.featAverage);
    ...
    function generateEmojis() {
        const emojiHelpers = (function () {
            function popularityEmoji() {
                if (TRACK_FEATS.popularity.mean >= FEAT_HIGH) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.sheepEmoji));
                }
                else if (TRACK_FEATS.popularity.mean <= FEAT_LOW) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.wolfEmoji));
                }
                else {
                }
            }
            function valenceEmoji() {
                if (TRACK_FEATS.valence.mean >= FEAT_HIGH) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.happyEmoji));
                }
                else if (TRACK_FEATS.valence.mean <= FEAT_LOW) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.sad));
                }
                else {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.neutralEmoji));
                }
            }
            function acousticEmoji() {
                if (TRACK_FEATS.acousticness.mean >= FEAT_HIGH) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.acousticEmoji));
                }
                else if (TRACK_FEATS.acousticness.mean <= FEAT_LOW) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.nonAcousticEmoji));
                }
                else {
                }
            }
            function instrumentalEmoji() {
                if (TRACK_FEATS.instrumentalness.mean >= FEAT_HIGH) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.instrumentEmoji));
                }
                else if (TRACK_FEATS.instrumentalness.mean <= FEAT_LOW) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.singerEmoji));
                }
                else {
                }
            }
            function danceEmoji() {
                if (TRACK_FEATS.danceability.mean >= FEAT_HIGH) {
                    emojiContainer.appendChild(getEmojiHtml(config_js_1.config.PATHS.dancingEmoji));
                }
        ...
        const emojiContainer = document.getElementById(config_js_1.config.CSS.IDs.emojis);
        (0, config_js_1.removeAllChildNodes)(emojiContainer);
...
const addEventListeners = (function () {
    const selections = {
        featureTabManager: new SelectableTabEls_js_1.default(document
            .getElementById(config_js_1.config.CSS.IDs.featureSelections)
        ...
        termTabManager: new SelectableTabEls_js_1.default(document
            .getElementById(config_js_1.config.CSS.IDs.tracksTermSelections)
            .getElementsByTagName("button")[0], // first tab is selected first by default
        document
            .getElementById(config_js_1.config.CSS.IDs.tracksTermSelections)
            .getElementsByClassName(config_js_1.config.CSS.CLASSES.borderCover)[0] // first tab is selected first by default
    ...
    function addTrackFeatureButtonEvents() {
        function onClick(btn, borderCover) {
            const feature = btn.getAttribute(config_js_1.config.CSS.ATTRIBUTES.dataSelection);
            let selectedFeat = featureManager.TRACK_FEATS[feature];
            if (selectedFeat == undefined) {
                console.error("The selected attribute " +
                    feature +
                    " from " +
                    config_js_1.config.CSS.ATTRIBUTES.dataSelection +
            ...
            featureManager.updateTracksChart(currTracks.slice(0, trackActions.selections.numViewableCards));
        }
    ...
    function addTrackTermButtonEvents() {
        function onClick(btn, borderCover) {
            trackActions.selections.term = btn.getAttribute(config_js_1.config.CSS.ATTRIBUTES.dataSelection);
    ...
    function addExpandDescOnHoverEvents() {
        ...
        const descDivs = document.getElementsByClassName(config_js_1.config.CSS.CLASSES.chartInfo)[0].children;
        for (let i = 0; i < descDivs.length; i++) {
            // obtain the description and the drop down button
            let desc = descDivs[i];
            let textContainer = desc.getElementsByClassName(config_js_1.config.CSS.CLASSES.expandableTxtContainer)[0];
            if (!textContainer) {
                continue;
            }
            let ellipsisText = textContainer.children[0];
            desc.addEventListener("mouseenter", () => {
                ellipsisText.classList.remove(config_js_1.config.CSS.CLASSES.ellipsisWrap);
                desc.style.flexBasis =
                    TOP_PADDING + BTM_PADDING + textContainer.offsetHeight + "px";
            });
            desc.addEventListener("mouseleave", () => {
                ellipsisText.classList.add(config_js_1.config.CSS.CLASSES.ellipsisWrap);
