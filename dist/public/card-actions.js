...
class CardActionsHandler {
     ...
     */
    onCardClick(selCardEl, corrObjList, callback = null, allowUnselSelected = false, unselectPrevious = true) {
        if (this.storedSelEls.includes(selCardEl)) {
            if (allowUnselSelected) {
                ...
                selCard.classList.remove(config_js_1.config.CSS.CLASSES.selected);
      ...
      is assigned as the card elements HTML 'id' when the card is created.");
        ...
        if (Object.keys(this.storedSelEls).length > 0 && unselectPrevious) {
            this.storedSelEls.pop().classList.remove(config_js_1.config.CSS.CLASSES.selected);
        }
        // on click add the 'selected' class onto the element which runs a transition
        selCardEl.classList.add(config_js_1.config.CSS.CLASSES.selected);
    ...
    scrollTextOnCardEnter(enteringCardEl) {
        let scrollingText = enteringCardEl.getElementsByClassName(config_js_1.config.CSS.CLASSES.scrollingText)[0];
        let parent = scrollingText.parentElement;
        if ((0, config_js_1.isEllipsisActive)(scrollingText)) {
            parent.classList.add(config_js_1.config.CSS.CLASSES.scrollLeft);
            scrollingText.classList.remove(config_js_1.config.CSS.CLASSES.ellipsisWrap);
    ...
    runScrollingTextAnim(scrollingText, cardEl) {
        ...
        this.currScrollingAnim = scrollingText.animate([
            ...
            {
                transform: `translateX(${-(0, config_js_1.getTextWidth)(scrollingText.textContent, font) - LINGER_AMT}px)`,
            },
        ], {
            // timing options
            duration: 5000,
    ...
    scrollTextOnCardLeave(leavingCardEl) {
        var _a;
        let scrollingText = leavingCardEl.getElementsByClassName(config_js_1.config.CSS.CLASSES.scrollingText)[0];
        let parent = scrollingText.parentElement;
        parent.classList.remove(config_js_1.config.CSS.CLASSES.scrollLeft);
        scrollingText.classList.add(config_js_1.config.CSS.CLASSES.ellipsisWrap);
